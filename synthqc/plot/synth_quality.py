...
def plot_synth_quality_bar(synth_dirs, truth_dirs, norm_algs, synth_algs, out_dir=None, mask_dir=None, outtype='png'): # pragma: no cover
    ...
    if len(synth_algs) != len(synth_dirs) or len(norm_algs) != len(truth_dirs_):
        raise SynthQCError('Number of algorithm names must equal number of directories provided')
    ...
    for i, (synth_dir_, synth_alg_) in enumerate(zip(synth_dirs, synth_algs)):
        ...
        for j, (norm_alg_, synth_dir, truth_dir) in enumerate(zip(norm_algs, synth_dirs_, truth_dirs_)):
            ...
            if len(synth_fns) != len(truth_fns) or len(synth_fns) == 0:
                raise SynthQCError('Number of synthesized and truth images must be equal and non-zero')
            if mask_dir is None:
                mask_fns = [None] * len(synth_fns)
            else:
                mask_fns = glob_nii(mask_dir)
                if len(synth_fns) != len(mask_fns):
                    raise SynthQCError('Number of images and masks must be equal and non-zero')
            masks = [None if mask_fn is None else nib.load(mask_fn).get_data() for mask_fn in mask_fns]
            synth = [nib.load(synth_fn).get_data() if mask is None else nib.load(synth_fn).get_data() * mask
                     for synth_fn, mask in zip(synth_fns, masks)]
            truth = [nib.load(truth_fn).get_data() if mask is None else nib.load(truth_fn).get_data() * mask
                     for truth_fn, mask in zip(truth_fns, masks)]
            for syn, th, mask in zip(synth, truth, masks):
                stats_, metrics_ = synth_quality(syn, th, mask, use_mi=True)
                stats.extend(stats_)
...
def plot_dir_synth_quality(synth_dir, truth_dir, out_dir=None, mask_dir=None, outtype='png', mean=False):
    ...
    if not mean:
        for i, (synth_fn, truth_fn, mask_fn) in enumerate(zip(synth_fns, truth_fns, mask_fns), 1):
            ...
            mask = None if mask_fn is None else nib.load(mask_fn).get_data()
            synth = nib.load(synth_fn).get_data() if mask is None else nib.load(synth_fn).get_data() * mask
            truth = nib.load(truth_fn).get_data() if mask is None else nib.load(truth_fn).get_data() * mask
...
def plot_synth_quality(synth, truth, mask, mean=False):
    ...
    """
    if not mean:
        stats, metrics = synth_quality(synth, truth, mask if mask is not None else None)
        ...
        if not isinstance(synth, list) or not isinstance(truth, list) or not isinstance(mask, list):
            raise SynthQCError('If mean option used, then arguments must be lists (of equal size) '
        ...
        for i, (synth_, truth_, mask_) in enumerate(zip(synth, truth, mask)):
            ...
            stats_, metrics = synth_quality(synth_, truth_, mask_ if mask_ is not None else None)
